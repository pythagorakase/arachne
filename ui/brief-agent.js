...
(() => {
  ...
  function isTextEntryControl(control) {
    ...
    return tag === "input" && TEXT_INPUT_TYPES.has(normalizedType(control));
  }
  ...
  let focusedControlFramePending = false;
  let focusedTextControl = null;
  ...
  function keepFocusedControlVisible() {
    focusedControlFramePending = false;
  ...
  function scheduleFocusedControlVisibility() {
    if (focusedControlFramePending) return;
    focusedControlFramePending = true;
    window.requestAnimationFrame(keepFocusedControlVisible);
  }

  function handleFocusedControlResize() {
    if (focusedTextControl) scheduleFocusedControlVisibility();
  }
  ...
  window.addEventListener("resize", handleFocusedControlResize);
  document.addEventListener("focusin", (event) => {
    if (!isTextEntryControl(event.target)) return;
    focusedTextControl = event.target;
    scheduleFocusedControlVisibility();
  });
  document.addEventListener("focusout", (event) => {
    if (event.target === focusedTextControl) focusedTextControl = null;
  });
